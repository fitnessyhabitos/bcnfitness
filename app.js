...
const admin = {
    ...
    viewClient: async (uid) => {
        ...
        if(chkFolds) chkFolds.checked = perms.folds || false;
        if(chkMeas) chkMeas.checked = perms.meas || false;
...
const profile = {
    render: () => {
        ...
        // --- MOSTRAR SOLO SI TIENE PERMISOS (DEFENSIVO) ---
        const perms = state.profile.perms || {}; // Si no existe, es objeto vacio
        ...
        // Pliegues
        if(perms.folds) {
            if(btnFolds) btnFolds.classList.remove('hidden');
            if(cardChartFolds) cardChartFolds.classList.remove('hidden');
        } else {
            if(btnFolds) btnFolds.classList.add('hidden');
            if(cardChartFolds) cardChartFolds.classList.add('hidden');
        }

        // Medidas
        if(perms.meas) {
            if(btnMeas) btnMeas.classList.remove('hidden');
            if(cardChartMeas) cardChartMeas.classList.remove('hidden');
        } else {
            if(btnMeas) btnMeas.classList.add('hidden');
            if(cardChartMeas) cardChartMeas.classList.add('hidden');
        }
    ...
    saveStats: async () => {
        ...
        const foldsDiv = document.getElementById('folds-inputs');
        if(foldsDiv && !foldsDiv.classList.contains('hidden')) {
             const folds = {
        ...
        const measDiv = document.getElementById('measures-inputs');
        if(measDiv && !measDiv.classList.contains('hidden')) {
             const measures = {
