...
const app = {
    init: () => {
        // Registro de Service Worker para Notificaciones Web Push
...
            window.chartHelpers.renderLine('clientMuscleChart', lineData, 'muscle', '#00d4ff');
            ...
            // Cargar gráfica JP7 si está activa
            if(user.settings?.showJP7 && user.jp7History) {
                document.getElementById('client-jp7-card').classList.remove('hidden');
                const jp7Data = [...user.jp7History].sort((a,b) => (a.date.seconds||new Date(a.date)) - (b.date.seconds||new Date(b.date)));
                window.chartHelpers.renderJP7('clientJP7Chart', jp7Data);
            } else {
                document.getElementById('client-jp7-card').classList.add('hidden');
            }
        }
...
                if(navigator.vibrate) navigator.vibrate([200,200]);
                // Notificación Push para el descanso
                profile.sendPushNotification("¡Descanso terminado!", "Es hora de la siguiente serie.");
...
        document.getElementById('conf-rest-time').value = state.profile.settings?.restTime || 60;
        ...
        // Mostrar/Ocultar secciones habilitadas por el Coach
...
    },
    saveJP7: async () => {
        ...
        if(!data.pecho) return alert("Introduce al menos el pecho");
...
        state.profile.settings = { ...state.profile.settings, weeklyGoal: g, restTime: r }; alert("Guardado"); dashboard.render();
    },
    loadHistory: async () => {
...
    requestNotify: () => { 
        Notification.requestPermission().then(permission => {
            if(permission === 'granted') {
                app.showToast("Notificaciones habilitadas");
                profile.sendPushNotification("¡Genial!", "Las notificaciones están listas.");
            }
        }); 
    },
    sendPushNotification: (title, body) => {
        ...
        navigator.serviceWorker.ready.then(registration => {
            registration.showNotification(title, {
                ...
                icon: 'logo.png',
                vibrate: [300, 100, 300],
                tag: 'bcn-fitness-push',
                renotify: true,
                requireInteraction: true
            });
        });
    },
    testSound: () => { if(state.sounds.beep) { state.sounds.beep.currentTime = 0; state.sounds.beep.play(); } profile.sendPushNotification("Test de Aviso", "Esta notificación sonará siempre."); },
    uploadPhoto: (input) => {
...
    },
    renderJP7: (id, data) => {
        ...
        new Chart(ctx, {
            type: 'line',
            data: {
                labels: data.map(d => new Date(d.date.seconds*1000 || d.date).toLocaleDateString()),
                datasets: [
                    { label: 'Abdominal', data: data.map(d => d.abdo), borderColor: '#39ff14', tension: 0.3 },
                    { label: 'Muslo', data: data.map(d => d.muslo), borderColor: '#ff3b30', tension: 0.3 }
                ]
            },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: true, labels: { color: 'white', size: 10 } } } }
        });
    }
};
